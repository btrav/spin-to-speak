...
const SpinnerWheel: React.FC<SpinnerWheelProps> = ({ participants, isSpinning, darkMode, spinRotation }) => {
  const colors = [
  ...
  if (participants.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center">
        <div className={`w-80 h-80 rounded-full border-4 border-dashed flex flex-col items-center justify-center relative overflow-hidden ${
  ...
  // For single participant, create a full circle WITHOUT center circle
  if (participants.length === 1) {
    return (
      <div className="flex flex-col items-center justify-center">
        <div className="relative">
          {/* Black Triangle Pointer - POINTING DOWN */}
          <div className="absolute top-0 left-1/2 transform -translate-x-1/2 -translate-y-4 z-20">
            <div className="w-0 h-0 border-l-[12px] border-r-[12px] border-t-[20px] border-l-transparent border-r-transparent border-t-black drop-shadow-lg"></div>
          </div>
          
          {/* Single participant wheel - NO CENTER CIRCLE */}
          <div className="relative">
            <div 
              className={`w-80 h-80 rounded-full flex items-center justify-center shadow-2xl border-8 border-white transition-transform duration-[3000ms] ease-out`}
              style={{
                background: colors[0],
            ...
            {/* NO CENTER CIRCLE FOR SINGLE PARTICIPANT */}
          </div>
        </div>
    ...
    <div className="flex flex-col items-center justify-center">
      <div className="relative">
        ...
        <div className="relative">
          ...
          {/* Center circle - ONLY for multiple participants */}
          <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-20 h-20 bg-white rounded-full shadow-xl border-4 border-gray-200 flex items-center justify-center z-10">
            <div className="w-10 h-10 bg-gradient-to-r from-blue-500 to-purple-500 rounded-full animate-pulse-slow"></div>
          </div>
        </div>
      </div>
